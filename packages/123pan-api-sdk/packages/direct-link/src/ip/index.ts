...
export class IpModule {
  ...
  async toggleBlacklist(
    params: ToggleIpBlacklistParams
  ): Promise<ApiResponse<ToggleIpBlacklistResponse>> {
    const { Status } = params
    ...
    return this.httpClient.post<ToggleIpBlacklistResponse>(
      '/api/v1/developer/config/forbide-ip/switch',
      {
        Status
      }
    )
  ...
  async updateBlacklist(params: UpdateIpBlacklistParams): Promise<ApiResponse<{}>> {
    ...
    if (invalidIps.length > 0) {
      throw new Error(
        `以下IP地址格式不正确：${invalidIps.slice(0, 5).join(', ')}${invalidIps.length > 5 ? '...' : ''}`
      )
    }
