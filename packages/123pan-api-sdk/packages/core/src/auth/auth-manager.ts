...
export class AuthManager {
  ...
  constructor(config: SdkConfig) {
    ...
    this.baseURL = config.baseURL || 'https://www.123pan.cn/b'
    ...
    const cacheDir = config.cacheConfig?.cacheDir || path.join(os.homedir(), '.123pan-sdk')
    ...
    if (this.cacheEnabled && !fs.existsSync(cacheDir)) {
      try {
        ...
        this.logger.warn('Failed to create cache directory, caching disabled', error as Error)
  ...
  async generateQrLogin(): Promise<QrLoginSession> {
    const response = await this.httpClient.get<ApiResponse<{ uniID?: string; url?: string }>>(
  ...
  private async initializeCacheManager(): Promise<void> {
    try {
      ...
      this.logger.warn('Failed to initialize multi-level cache manager', error as Error)
    }
  }
  ...
  async forceRefreshTokenWithRetry(maxRetries: number = 3): Promise<string> {
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        this.logger.info(`Attempting token refresh (attempt ${i + 1}/${maxRetries})`)
  ...
  private loadTokenFromCache(): void {
    ...
    try {
      ...
      this.logger.warn('Failed to load token from cache, will fetch new one', error as Error)
      this.deleteCacheFile()
    }
  }
  ...
  private saveTokenToCache(): void {
    ...
    try {
      fs.writeFileSync(this.cacheFilePath, JSON.stringify(this.tokenInfo, null, 2), 'utf-8')
